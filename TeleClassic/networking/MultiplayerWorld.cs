...
namespace TeleClassic.Networking
{
    public partial class MultiplayerWorld : World
    {
        ...
        protected Dictionary<PlayerSession, sbyte> playerIdMap;
        protected Dictionary<sbyte, PlayerSession> idPlayerMap;
        ...
        public MultiplayerWorld(string name, string fileName, Permission minimumBuildPerms, Permission minimumJoinPerms, int playerCapacity) : base(name, fileName)
        {
            ...
            idPlayerMap = new Dictionary<sbyte, PlayerSession>(idCap);
        ...
        public virtual void TeleportPlayer(PlayerSession playerSession, PlayerPosition playerPosition)
        {
            if (!playerJoinMode.ContainsKey(playerSession))
                throw new InvalidOperationException("Player has not joined world.");

            this.UpdatePosition(playerSession, playerPosition);
            playerSession.SendPacket(new PositionAndOrientationPacket(-1, playerPosition));
        }
