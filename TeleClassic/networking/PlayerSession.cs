...
namespace TeleClassic.Gameplay
{
    ...
    {
        SuperForthInstance.Machine.ForeignFunction getPlayerInfoDelegate;
        SuperForthInstance.Machine.ForeignFunction getPlayerPositionDelegate;
        SuperForthInstance.Machine.ForeignFunction teleportPlayerDelegate;

        private int FFIGetPlayerInfo(ref SuperForthInstance.Machine machine, ref SuperForthInstance.MachineRegister input, ref SuperForthInstance.MachineRegister output)
        {
            if (!configured)
            {
                Logger.Log("minigame/error", "Minigame tried to fetch player info before finishing configuration.", this.Name);
                return 0;
            }
            if (!handlePlayerMap.ContainsKey(input.LongInt))
            {
                Logger.Log("minigame/error", "Minigame supplied invalid player handle whilst fetching player info.", this.Name);
                return 0;
            }

            PlayerSession playerSession = handlePlayerMap[input.LongInt];
            output = SuperForthInstance.MachineRegister.NewHeapAlloc(ref machine, 2, SuperForthInstance.MachineHeapAllocation.GCTraceMode.Some);
            SuperForthInstance.MachineHeapAllocation playerInfo = output.HeapAllocation;
            playerInfo.ConfigureCustomGCTraceSchema(true, false);

            playerInfo[0] = SuperForthInstance.MachineRegister.FromString(ref machine, playerSession.Name);
            playerInfo[1] = SuperForthInstance.MachineRegister.FromInt((int)playerSession.Permissions);

            return 1;
        }

        private int FFIGetPlayerPosition(ref SuperForthInstance.Machine machine, ref SuperForthInstance.MachineRegister input, ref SuperForthInstance.MachineRegister output)
        {
            if (!configured)
            {
                Logger.Log("minigame/error", "Minigame tried to fetch player info before finishing configuration.", this.Name);
            ...
            PlayerSession playerSession = handlePlayerMap[input.LongInt];
            PlayerPosition position = GetPlayerPosition(playerSession);

            output = SuperForthInstance.MachineRegister.NewHeapAlloc(ref machine, 5, SuperForthInstance.MachineHeapAllocation.GCTraceMode.None);
            SuperForthInstance.MachineHeapAllocation playerPos = output.HeapAllocation;

            playerPos[0] = SuperForthInstance.MachineRegister.FromInt(position.X);
            playerPos[1] = SuperForthInstance.MachineRegister.FromInt(position.Y);
            playerPos[2] = SuperForthInstance.MachineRegister.FromInt(position.Z);
            playerPos[3] = SuperForthInstance.MachineRegister.FromInt(position.Heading);
            playerPos[4] = SuperForthInstance.MachineRegister.FromInt(position.Pitch);

            return 1;
        }

        private int FFITeleportPlayer(ref SuperForthInstance.Machine machine, ref SuperForthInstance.MachineRegister input, ref SuperForthInstance.MachineRegister output)
        {
            if (!configured)
            {
                Logger.Log("minigame/error", "Minigame tried to fetch player info before finishing configuration.", this.Name);
            ...
            PlayerSession playerSession = handlePlayerMap[input.LongInt];
            PlayerPosition newPosition = new PlayerPosition(input.HeapAllocation);

            if(!InWorld(new BlockPosition(newPosition)))
            {
                output.BoolFlag = false;
                return 1;
            }

            this.TeleportPlayer(playerSession, newPosition);
